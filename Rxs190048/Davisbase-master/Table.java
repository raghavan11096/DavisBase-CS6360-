...
public class Table{
	...
	public static String[] getColName(String table){ //tables=davisbase_tables
		...
		try{
			...
			filter(file, cmp, columnName, tempStorage);
			HashMap<Integer, String[]> content = tempStorage.content;
	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnName, TempStorage tempStorage){
		try{
			...
			for(int page = 1; page <= numOfPages; page++){
				...
				if(pageType == 0x0D)
				{
					...
					for(int i=0; i < numOfCells; i++){
						...
						if(check)
							tempStorage.add_vals(rowid, vals);
					}
				}
				else
					continue;
			...
			tempStorage.columnName = columnName;
			tempStorage.format = new int[columnName.length];
...
public static int searchKeyPage(RandomAccessFile file, int key){
	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnName, String[] type, TempStorage tempStorage){
		try{
